...
class TestCLI:
    ...
    def test_smoke_apply_calls_hermes_with_triage_no_assignee(self, mod, valid_draft_builder, tmp_path):
        """--smoke-apply must call hermes, use --triage, and omit --assignee.

        The key smoke-safety fix: even if the task draft has assignee=aed-builder,
        --smoke-apply must create the task with no assignee so the dispatcher
        cannot auto-claim it.
        """
        draft_path = tmp_path / "draft.json"
        draft_path.write_text(json.dumps(valid_draft_builder))
        out_json = tmp_path / "plan.json"
