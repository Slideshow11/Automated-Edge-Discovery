...
def build_runner_output(
    ...
    # Missing-value summary (observation_table_missing_value_summary audit)
    if has_missing_value_summary_requested and manifest is not None:
        # DatasetManifest is a single dataset object; access source_kind and path directly
        if manifest.source_kind.value != "local_csv":
            missing_val_fail_audit = {
                "audit_name": "observation_table_missing_value_summary",
                "audit_result": "fail",
                ...
                "blocker_count": 1,
                "warning_count": 0,
                "details_ref": "No CSV dataset found in DataManifest for missing-value summary.",
                "created_at": _utc_now(),
            }
            new_audits = list(audit_summary["audits"])
            new_audits.append(missing_val_fail_audit)
            ...
            # Trigger GovernanceRejection via blocker check below
            _no_csv_artifact = {
                "runner_output_id": RUNNER_OUTPUT_ID_DEFAULT,
                "runner_output_version": RUNNER_OUTPUT_VERSION,
                "run_id": run_id,
                ...
                "failure_summary": {
                    ...
                    "failed_check": "observation_table_missing_value_summary",
                    "blocker_summary": (
                        "observation_table_missing_value_summary: "
                        "No CSV dataset found in DataManifest for missing-value summary."
                    ),
                    "missing_data_summary_ref": None,
                    "details_ref": None,
                    "created_at": created_at,
            ...
            raise GovernanceRejection(
                _no_csv_artifact,
                "Run blocked: no CSV dataset found for missing-value summary.",
            )
        ...
        try:
            ...
            missing_val_details = missing_val_result["details"]
            # Missing values found → fail closed
            any_missing = any(
                count > 0 for count in missing_val_result["missing"].values()
            )
            if any_missing:
        ...
        if not missing_val_passed and audit_summary["blocker_count"] > 0:
            failing_checks = [
                a["audit_name"] for a in audit_summary["audits"]
                if a["audit_result"] == "fail"
            ]
            ...
            raise GovernanceRejection(
                failed_artifact,
                f"Run blocked by observation_table_missing_value_summary failure: "
                f"{missing_val_details}.",
            )
