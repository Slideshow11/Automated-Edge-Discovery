...
1. `run_guarded_pr_flow.py` (Historical note: this tool was retired by #410.) produced `READY_TO_MERGE_CANDIDATE`.
...
| **Gate runner** | Humphry / M3 | Executes `run_guarded_pr_flow.py` (Historical note: this tool was retired by #410.), `wait_for_pr_ready.py`, scope_guard, PMG |
| **PR controller** | Humphry / M3 | Watches PR state, runs deterministic checks, emits gate state |
| **Merge authority** | Guarded flow | `run_guarded_pr_flow.py` (Historical note: this tool was retired by #410.) + `check_merge_authorization.py` + human phrase + `--match-head-commit` |
