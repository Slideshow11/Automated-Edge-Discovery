...
- PR #206 fix: harden controller `--apply-create-task` with idempotency key, no-dispatch guarantee, and exactly-one invocation — `_make_controller_idempotency_key()` produces deterministic `aed:{owner}/{repo}:pr:{n}:head:{sha}:action:{action}` key; `_apply_create_task()` refuses for `no_action_wait`, `ci_pending`, `codex_pending`, `unknown`, or missing required fields, surfacing blockers in run packet; run_packet gains `apply_create_task_requested`, `apply_create_task_allowed`, `idempotency_key`, `downstream_helper`, `no_dispatch_guarantee: True` fields; exactly one kanban helper invocation; 17 new tests; controller never dispatches, merges, or calls Codex from within its own process
- PR #207 feat: add final-head review evidence packet for exact-SHA merge gating — `aed.pr_gate.review_evidence.v1` packet with current_head_sha, reviewed_head_sha, review_source, review_status, review_is_stale, ci_all_green, scope_status, merge_allowed fields; exact-head rule enforced: `review_is_stale` is True when reviewed_head_sha != current_head_sha; merge_allowed is False when review_is_stale is True; GitHub Codex and Codex CLI fallback evidence requires exact-head match and clean status; missing/pending/suggestions review evidence blocks merge; CI must be all-green; scope must be clean; `check_merge_authorization.py` extended with `--review-evidence` argument that rejects stale/missing evidence, ci_not_all_green, scope_not_clean, review_status_not_clean; `pr_gate_merge_ready_notify.py` extended with `--review-evidence` argument that includes review_source, reviewed_head_sha, current_head_sha, review_is_stale, ci_all_green, scope_status in notification output and refuses authorization phrase when evidence stale or merge_allowed=False; `build_merge_ready_packet.py` extended with `--build-review-evidence` mode; 16 new tests in test_merge_authorization_guard.py, 10 new tests in test_pr_gate_merge_ready_notify.py; docs/merge_authorization_guard.md added
- PR #208 feat: add exact full-SHA authorization enforcement in merge authorization guard — `authorization_sha_mismatch` blocker when phrase SHA != confirmed PR head; short SHA prefixes (7, 39 chars) rejected; substitution forbidden: agent must never swap user-provided SHA for actual head; `check_authorization_sha_match()` added to `check_merge_authorization.py`; packet gains `authorization_head_sha` and `head_sha_source` fields from `build_merge_ready_packet.py`; 13 new tests in test_merge_authorization_guard.py; docs/merge_authorization_guard.md updated
- PR #209 feat: add mechanical PR scope diff enforcement — `check_pr_scope.py` compares changed_files against allowed_files and forbidden_files without relying on prompts; `aed.pr_gate.scope_check.v1` packet; glob patterns (docs/**, scripts/local/*.py) supported; `scope_status` = clean | violation | unknown; blockers: allowed_files_missing, changed_file_outside_allowed_scope, forbidden_file_touched; exit 0=clean, 1=violation, 2=bad args; tests/test_check_pr_scope.py created; docs/merge_authorization_guard.md and docs/README.md updated
- PR #210 ci: cancel stale PR workflow runs with concurrency — add GitHub Actions workflow-level concurrency to .github/workflows/ci.yml: group `${{ github.workflow }}-${{ github.event.pull_request.number || github.ref }}`, cancel-in-progress `${{ github.ref != 'refs/heads/main' }}`; cancels stale in-progress runs on PR branches to save Actions minutes; main branch runs are never cancelled; validate_ci_workflow_invariants.py extended with 6 new concurrency invariants (top-level concurrency block, group exists, group includes github.workflow, cancel-in-progress exists, cancel-in-progress protects main); 9 new tests added; docs/README.md and docs/current_project_status.md updated
