...
| 7 | Codex review | PR at the live head | Codex review submission + inline findings | `scripts/local/check_pr_review_comments.py` (severity-classified) and the canonical `scripts/local/audit_codex_response_for_pr.py` classifier (lifecycle-classified) after a `@codex review` ping. The classifier is the source of truth for the lifecycle state; it scans BOTH PR-level issue comments (where Codex frequently posts clean passes) AND formal review submissions, and reports a bounded lifecycle state. | Inspect severity-classified findings; report the classifier's lifecycle state | `CODEX_CLEAN_PASS_RESOLVE_ONLY_NEEDED` on a clean current-head pass with no open P0/P1/P2; `HOLD_NEW_CODEX_THREAD` if Codex raises a new current-head finding |
