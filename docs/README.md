...
- Phase-ledger merge-readiness stack RETIRED in AED Consolidation Pass 2 (merged via canonical controller scripts/local/aed_pr.py). The historical record of its prior 4-PR sequence (PRs #390, #391, #392, #393) is preserved below as a one-line history reference; the procedural sections of `docs/phase_ledger_merge_readiness_wrapper.md` no longer describe the canonical path. Operator guide now lives in `docs/aed_pr_canonical_guide.md`.
...
- Resume checkpoint continuation rule codified (2026-06-10): a new canonical `HOLD_RESUME_CHECKPOINT_NEEDED` state in `schemas/aed_lifecycle_states_v1.json` codifies the rule that continuation turns must resume from the latest verified lifecycle state and must not restart broad planning, repeat completed phases, or repeat already-authorized mutations. Before any mutation, the operator must verify (using read-only checks) the current PR number and URL, current head SHA, current lifecycle state, completed phases, remaining permitted mutations, already-performed mutations, and protected PR/worktree state. If the state cannot be reconstructed from durable evidence, stop and report the hold. Docs-only: no script behavior changes. Cross-references added in `docs/aed_lifecycle_state_registry.md` §11 (new), `docs/aed_whole_workflow_operator_path.md` §8 (new) with renumbering of "Lessons from PR #394" to §9 and "Where next work belongs" to §10, and `docs/aed_known_safe_command_cookbook.md` §11.2 (new) plus §14 state-to-command index. 19 new tests in `tests/test_aed_lifecycle_states.py` covering the entry's description, evidence, allowed_next_states, and forbidden_mutations contract, plus a cross-reference regression guard.
- Primary worktree sync policy codified (2026-06-10): the rule that the primary worktree at `/home/max/Automated-Edge-Discovery` is intentionally left stale at the post-closeout head of the last merged PR (and is **not** a mirror of `origin/main` and **not** updated as a side effect of any governance PR) is now stated once in canonical policy vocabulary. Every governance PR run is performed in a dedicated temp worktree branched from `origin/main`; the primary worktree is read-only with respect to the governance path. The PHASE 1 verification pattern (`status --porcelain` empty, `HEAD` equals expected post-closeout head, `branch --show-current` is `main`) is now documented in three places, all cross-referenced. The canonical `HOLD_MAIN_HEAD_MISMATCH` state's `forbidden_mutations` list already includes `worktree_update`; the policy text restates which repository-side actions (pull, fetch+checkout, reset, checkout, branch switch, worktree removal) are forbidden in the primary during a governance PR run. Docs-only: no script behavior changes. Cross-references added in `docs/aed_lifecycle_state_registry.md` §13 (new), `docs/aed_whole_workflow_operator_path.md` §9 (new) with renumbering of "Lessons from PR #394" to §10 and "Where next work belongs" to §11, and `docs/aed_known_safe_command_cookbook.md` §11.3 (new) plus §13 forbidden patterns and §14 state-to-command index. New tests in `tests/test_aed_lifecycle_states.py` covering the new section numbers, the canonical `HOLD_MAIN_HEAD_MISMATCH.forbidden_mutations.worktree_update` invariant, the operator path §5 authority table cross-reference, and the cookbook §13/§11.3 cross-references.
- Codex response classifier added (PR #402): `scripts/local/audit_codex_response_for_pr.py` is a read-only classifier that scans BOTH PR-level issue comments (where Codex frequently posts clean passes) AND formal review submissions, inventories review threads, and reports a bounded lifecycle state. The classifier is the canonical source of truth for Codex response classification in future PR lifecycle prompts; an operator loop that watches only formal review submissions will miss Codex clean-pass comments and report `HOLD_CODEX_RESPONSE_PENDING` indefinitely. Adds 24 focused tests, two narrow lifecycle state descriptions (`CODEX_CLEAN_PASS` informational and `HOLD_PR_NOT_OPEN` hold), and a §6.6 section in the known-safe command cookbook. The classifier performs only read operations against GitHub (no thread resolve, no review dismiss, no comment delete, no merge). Docs-only additions: §6.6 cookbook and §7 operator-path cross-reference.
...
| docs/aed_pr_canonical_guide.md | NewCanonicalController | ONE current guide for the canonical AED PR controller (scripts/local/aed_pr.py status/advance/merge). Replaces the per-step operator-path docs. | Active v1 |
...
| docs/aed_continue_pr.md | ContinuationLayer | `aed continue-pr --dry-run` CLI: read-only continuation workflow planner that emits a structured plan (current state, completed phases, remaining mutations, proposed merge command preview) without mutating GitHub. Implements dual-endpoint Codex waiter (formal reviews + PR-level issue comments) per the PR #405 lesson. Reuses the B2 source-aware review-comment gate via subprocess. Mandatory `--dry-run`; refuses `--execute`/`--force`/`--admin`/`--no-dry-run`. PR #407+ would add the corresponding `--execute` command. | Active v1 |
...
| scripts/local/aed_pr.py | Canonical AED PR-lifecycle controller: status (read live state) + advance (safe mechanical steps, no merge) + merge (requires canonical human phrase, executes squash merge with --match-head-commit). | Read + merge |
