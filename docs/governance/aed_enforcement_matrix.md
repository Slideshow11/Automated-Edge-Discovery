...
| AED-RULE-006 | Reject the admin-bypass and auto-merge flags | `merge_pr_safely.py::reject_admin`; `merge_readiness_with_phase_ledger.py`; `docs/phase_ledger_merge_readiness_wrapper.md` | hard-coded | Hard | `tests/test_merge_pr_safely.py`; `tests/test_merge_readiness_with_phase_ledger.py` | none | `AEDMergeTool` argparse + `AEDPolicy` |
